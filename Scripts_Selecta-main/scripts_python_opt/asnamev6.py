...
def extract_ipv6_and_indices(lines):
    """Extract IPv6 addresses and their indices from SNMP walk lines."""
    ipv6_dict = {}
    for line in lines:
        if "STRING: \"" in line:
            ...
            oid = parts[0]
            ip = parts[1].strip('"')
            if ".16." in oid:  # Ensure it's IPv6
                index = oid.split(".16.")[1]  # Extract parts after ".16."
                ipv6_dict[index] = ip
    return ipv6_dict
...
def main():
    ...
    ipv6_dict = extract_ipv6_and_indices(ip_results)
    asn_dict = extract_asns_and_indices(asn_results)

    output = {"data": []}

    for index, ip in ipv6_dict.items():
